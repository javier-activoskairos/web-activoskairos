...
const CASES = [
  {
    ...
    logoH: 64,
    dash: "dash-mintech.jpg",
    sector: "Fondo de Inversión",
    contexto:
      "Gestión de contratos en múltiples divisas, red de clientes y asesores",
    ubicacion: "Argentina",
    equipo: "+5",
    recorrido: ["Initia", "Sophos", "Tempo"],
    summary:
      "De procesos manuales y sistemas desconectados → a una operación multidivisa centralizada y automatizada.",
    metrics: [
  ...
  },
  {
    ...
    summary:
      "De la gestión manual de socios → a un sistema centralizado que se sincroniza y escala solo.",
    metrics: [
  ...
  },
  {
    ...
    summary:
      "De procesos sin estandarizar → a una operación documentada y automatizada lista para crecer.",
    metrics: [
  ...
  },
  {
    ...
    recorrido: ["Initia", "Flux", "Tempo"],
    summary:
      "De herramientas dispersas → a un ecosistema digital unificado con IA y portal del cliente.",
    metrics: [
  ...
  },
  {
    ...
    summary:
      "De un control financiero disperso → a una plataforma única con trazabilidad y alertas automáticas.",
    metrics: [
      ...
      <Container>
        <div style={{ maxWidth: 880, marginBottom: "var(--space-7)" }}>
          ...
          >
            ...
            devueltas a nuestros clientes,
            <br />
            <span style={{ color: KAIROS_ACCENT, fontWeight: 600 }}>
              5 operativas convertidas en activos que funcionan solos
            </span>
            .
          </Reveal>
        </div>
...
function Marquee({ items, reduce, onOpen, posRef }) {
  ...
  React.useEffect(() => {
    ...
    const tick = (now) => {
      ...
      if (!paused.current && !drag.current.on) {
        ...
        if (half.current && offset.current <= -half.current)
  ...
  const tileClick = (i) => {
    if (!drag.current.moved) onOpen(i % items.length);
  };
  ...
  return (
    <div
      ...
      onMouseEnter={() => {
        paused.current = true;
        posRef.current.paused = true;
      }}
      onMouseLeave={() => {
        paused.current = false;
        posRef.current.paused = false;
      }}
    ...
    >
      <span
        ...
        style={{
          ...
          borderRadius: "50%",
          background:
            "radial-gradient(circle, rgba(249,99,2,0.22), transparent 70%)",
          opacity: 0,
          transition: reduce ? "none" : "opacity 280ms ease",
          pointerEvents: "none",
        }}
      />
      <span
        className="kairos-tile-cta"
        style={{
          position: "absolute",
          top: 14,
          right: 14,
      ...
      >
        {c.recorrido.map((p) => (
          <img
            key={p}
            src={`/assets/products/${PRODUCTS[p]}`}
            alt=""
            aria-hidden="true"
            title={p}
            style={{
  ...
  return (
    <div ref={ref}>
      <button
        type="button"
        onClick={onBack}
        className="kairos-back"
        style={{
          display: "inline-flex",
          alignItems: "center",
          gap: 9,
          background: "none",
          border: "1px solid var(--border-strong, rgba(26,23,20,0.18))",
          borderRadius: 999,
  ...
  return (
    <article
      ...
      style={{
        ...
        display: "grid",
        gridTemplateColumns: "minmax(0, 30%) 1px minmax(0, 1fr)",
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="kairos-case-rule"
        style={{
          background: KAIROS_ACCENT_GRAD,
          borderRadius: 2,
          alignSelf: "stretch",
        }}
      />
      <div
        className="kairos-case-main"
        style={{
          display: "flex",
          flexDirection: "column",
          minHeight: 0,
      ...
      >
        ...
        >
          ...
          {c.metrics[1] && (
            <div
              style={{
                paddingLeft: "clamp(1.25rem, 2.4vw, 2.25rem)",
...
function CaseFooter({ quote, author }) {
  const text = quote
    ? `«${quote}»`
    : "«[Insertar aquí frase/reseña del cliente]»";
  const who = author || "[Nombre] · [Puesto]";
  return (
    <div
      style={{
        flexShrink: 0,
        marginTop: "clamp(0.9rem, 1.6vw, 1.3rem)",
        paddingTop: "var(--space-4)",
        borderTop: "1px solid rgba(26,23,20,0.08)",
      }}
    >
      <p
        style={{
          ...
          color: quote ? "var(--text-strong)" : KAIROS_BODY,
          lineHeight: 1.4,
          textWrap: "balance",
        }}
      >
